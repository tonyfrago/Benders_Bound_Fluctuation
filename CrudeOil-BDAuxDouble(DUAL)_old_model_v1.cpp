...
double UpperBoundAuxLess = BigM;
double UpperBoundGlobalAuxLess = BigM;
double Gap = 1;
double fraction = 1;
...
int BDFeasCutsFromAux = 0, BDFeasCutsFromAuxLess = 0, BDFeasCutsFromFeasAux = 0, BDFeasCutsFromClass = 0;
int BDOptCutsFromAux = 0, BDOptCutsFromAuxLess = 0, BDOptCutsFromFeasAux = 0, BDOptCutsFromClass = 0;
...
char* FilePath_Results = "C:/PhD_Examples/Results_CrudeOil/BDAuxDouble(DUAL)_new";//G:/Antonis/PhD_Examples
...
IloModel modelDualSlaveAuxLess(env);
...
IloRangeArray AuxLessConl(env, 0);
...
IloObjective Dual_objective_AuxLess(env, 0);
...
vector <double> UpperBoundAuxLessArray;
vector <double> UpperBoundGlobalAuxLessArray;
vector <double> dTy;
...
vector <double> cTxAuxLess;
vector <double> bTuAuxLess;
...
int do_dual_slave_and_aux() {
		...
		for (z = 0; z < zmax; z++) {
			...
			for (j = 0; j < jmax; j++) {
				...
				for (t = 0; t < tmax; t++) {
					...
					modelDualSlaveAuxLess.add(Con1X);
		...
		for (k = 0; k < kmax; k++) {
			...
			for (j = 0; j < jmax; j++) {
				...
				for (t = 0; t < tmax; t++) {
					...
					modelDualSlaveAuxLess.add(Con2Y);
		...
		for (j = 0; j < jmax; j++) {
			...
			for (t = 0; t < tmax; t++) {
				if (t < tmax - 1) {
					...
					modelDualSlaveAuxLess.add(Con3I);
	...
	for (i = 0; i < imax; i++) {
		...
		for (z = 0; z < zmax; z++) {
			...
			for (t = 0; t < tmax; t++) {
				...
				modelDualSlaveAuxLess.add(Con4SC);
				Con4SCt.add(Con4SC);
				expr.end();
			}
	...
	for (k = 0; k < kmax; k++) {
		...
		for (z = 0; z < zmax; z++) {
			...
			for (t = 0; t < tmax; t++) {
				...
				modelDualSlaveAuxLess.add(Con5SD);
				Con5SDt.add(Con5SD);
				expr.end();
			}
	...
	for (l = 0; l < 1; l++) {
		...
		expr += 0;
		char AuxLessCon1[60];
		sprintf(AuxLessCon1, "AuxLessConl(l%d)", l);
		double LBAuxLessCon = -IloInfinity, UBAuxLessCon = 1;
		IloRange AuxLessCon(env, LBAuxLessCon, expr, UBAuxLessCon, AuxLessCon1);
		NoOfSlaveCons++;
		modelDualSlaveAuxLess.add(AuxLessCon);
		AuxLessConl.add(AuxLessCon);
		expr.end();
	}
	...
	*/
	...
	Dual_objective_AuxLess = IloAdd(modelDualSlaveAuxLess, IloMaximize(env, 0));
...
int UpdateDualSlaveAuxLessCon() {

	AuxLessConl[0].setExpr(Dual_objective.getExpr());
	AuxLessConl[0].setUB(LowestDualSlaveObjFunction);
...
int AuxLess_Dual_Slave_Unbounded(double *AuxLessDualSlaveObjFunction, double *AuxLessPrimalSlaveObjFunction) {
	//env.error() << "Auxiliary Dual Slave Problem is UNBOUNDED" << endl;
	//env.out() << "----------------------------------------------------------------" << endl;
	//------Upper Bound Global remains the same--------
	UpperBoundAuxLess = 100;
	UpperBoundGlobalAuxLess = UpperBoundGlobalAuxLess;

	*AuxLessPrimalSlaveObjFunction = 0;
	*AuxLessDualSlaveObjFunction = BigM;
	return 0;
}
int Dual_Slave_Bounded(IloCplex cplexDualSlave_ptr, double *DTransposeY, double *DualSlaveObjFunction, double *PrimalSlaveObjFunction, int *FirstTime_ptr, int loop_ptr, int *NoOfTimes_ptr, IloModel modelDualSlaveAux_ptr_ptr, IloModel modelDualSlaveAuxLess_ptr_ptr, int *NoofTimesWorseSolutionFound) {
	...
	*PrimalSlaveObjFunction = 0;
	...
	if (UpperBound >= UpperBoundGlobal) {//-----We found a no better feasible solution---
		...
		status = UpdateDualSlaveAuxLessCon();
		if (status != 0) {
			Found_Error("UpdateDualSlaveAuxLessCon");
			return -1;
		}
		//----------------------------------------------
		for (t = 0; t < tmax; t++) {
...
int AuxLess_Dual_Slave_Bounded(IloCplex cplexDualSlave_ptr, double *DTransposeY, double *AuxLessDualSlaveObjFunction, double *AuxLessPrimalSlaveObjFunction, int *FirstTime_ptr, int loop_ptr, int *NoOfTimes_ptr, IloModel modelDualSlaveAuxLess_ptr_ptr, int *NoofTimesWorseSolutionFound) {
	//env.error() << "Auxiliary Dual Slave Problem is BOUNDED" << endl;
	//env.error() << "Found a FEASIBLE solution of Slave LP" << endl;
	//env.out() << "----------------------------------------------------------------" << endl;
	*AuxLessDualSlaveObjFunction = cplexDualSlave_ptr.getObjValue();
	int status = 0;
	for (t = 0; t < tmax; t++) {
		for (z = 0; z < zmax; z++) {
			for (i = 0; i < imax; i++) {
	...
	*AuxLessPrimalSlaveObjFunction = 0;
	for (t = 0; t < tmax; t++) {
		for (z = 0; z < zmax; z++) {
			for (i = 0; i < imax; i++) {
				*AuxLessPrimalSlaveObjFunction += SCiztValue[i][z][t];
			}
			for (k = 0; k < kmax; k++) {
				*AuxLessPrimalSlaveObjFunction += SDkztValue[k][z][t];
			}
		}
	}

	UpperBoundAuxLess = *DTransposeY + *AuxLessPrimalSlaveObjFunction;


	if (UpperBoundAuxLess > UpperBoundGlobalAuxLess) {//-----We found a worse feasible solution---
		UpperBoundGlobalAuxLess = UpperBoundGlobalAuxLess;
	}
	else {//-----------We found a better feasible solution-------
		UpperBoundGlobalAuxLess = UpperBoundAuxLess;//Update Upper Bound
		OptimalOriginalObjFunction = UpperBoundGlobalAuxLess;
		OptimalMasterObjFunction = *DTransposeY;
		OptimalPrimalSlaveObjFunction = *AuxLessPrimalSlaveObjFunction;
		OptimalDualSlaveObjFunction = *AuxLessDualSlaveObjFunction;
		LowestDualSlaveObjFunction = OptimalDualSlaveObjFunction;
...
int AddBendersCutToMaster(int FeasOrOpt, IloExpr expr101, int loop, IloModel modelMaster_ptr, bool FromAux_ptr, bool FromAuxLess_ptr, bool FromFeasAux_ptr) {
	...
	if (FeasOrOpt == 0) {
		...
		else if (FromAuxLess_ptr == true) {
			sprintf(MasterCut, "FeasibilityCutFromAuxiliaryLessDSP(iter%d)", loop);
			BDFeasCutsFromAuxLess++;
		}
		...
		if (FromAux_ptr == true) {
			...
			sprintf(MasterCut, "OptimalityCutFromAuxiliaryLessDSP(iter%d)", loop);
			BDOptCutsFromAuxLess++;
		}
		else if (FromFeasAux_ptr == true) {
...
int BendersIteration(IloModel modelMaster_ptr, IloModel modelDualSlave_ptr, IloModel modelDualSlaveAux_ptr, IloModel modelDualSlaveAuxLess_ptr, IloModel modelDualSlaveFeasAux_ptr, int *loop_iter, int *NoofTimesWorseSolutionFound) {
	...
	IloCplex cplexDualSlaveAuxLess_ptr(env);
	...
	cplexDualSlaveAuxLess_ptr.extract(modelDualSlaveAuxLess_ptr);
	cplexDualSlaveAuxLess_ptr.exportModel("modelDualSlaveAuxLess.lp");
	...
	double AuxDualSlaveObjFunction = BigM, AuxPrimalSlaveObjFunction = 0, AuxLessDualSlaveObjFunction = BigM, AuxLessPrimalSlaveObjFunction = 0;
	...
	UpperBoundAuxLessArray.clear();
	UpperBoundGlobalAuxLessArray.clear();
	dTy.clear();
	zCurrent.clear();
	cTx.clear();
	bTu.clear();
	...
	BDFeasCutsFromAux = 0;
	BDOptCutsFromAux = 0;
	BDFeasCutsFromAuxLess = 0;
	BDOptCutsFromAuxLess = 0;
	...
	while (UpperBoundGlobal - LowerBound > epsilon && loop < 5000 && duration < maxhours * 3600) {
		...
		if (ClassicalDualStatus == 4) { //---------IF CLASSICAL DUAL SLAVE IS UNBOUNDED-----
			...
			if (FoundFeasibleSolution == true && SameMasterSolution == false && NeedClassicalDSP == false) {
				...
				if (DualStatus == 4) { //---------IF AUXILIARY DUAL SLAVE IS UNBOUNDED-----
					...
					FromAux = true;
					FromAuxLess = false;
					status = AddBendersCutToMaster(0, expr101, loop, modelMaster_ptr, FromAux, FromAuxLess, FromFeasAux);//if =0, then adds feasibility cut
					if (status != 0) {
						Found_Error("AddBendersFeasibilityCutToMaster");
						return -1;
					}
					...
					FromAux = true;
					FromAuxLess = false;
					status = AddBendersCutToMaster(1, expr101, loop, modelMaster_ptr, FromAux, FromAuxLess, FromFeasAux);//if =1, then adds optimality cut
					if (status != 0) {
				...
				//SOLVE AUXLESS (THE 2ND AUXILIARY DUAL SUBPROBLEM)

				//----------SOLVE THE AUXILIARY DUAL SLAVE PROBLEM AND ADD CUT TO MASTER
				status = UpdateDualSlaveObjective(Dual_objective_AuxLess);
				if (status != 0) {
					Found_Error("UpdateDualSlaveObjective");
					return -1;
				}

				/*status = UpdateDualSlaveAuxLessCon(&FirstTime, loop, &NoOfTimes, modelDualSlaveAuxLess_ptr);
				if (status != 0) {
					Found_Error("UpdateDualSlaveAuxLessCon");
					return -1;
				}*/
				...
				status = Solve_Dual_Slave(modelDualSlaveAuxLess_ptr, cplexDualSlaveAuxLess_ptr);
				if (status != 0) {
					Found_Error("Solve_Dual_Slave_AuxLess");
					return -1;
				}

				DualStatus = cplexDualSlaveAuxLess_ptr.getStatus();
				if (DualStatus == 3) {//---------IF DUAL SLAVE IS INFEASIBLE-----
					break;
				}
				...
				if (DualStatus == 4) { //---------IF AUXILIARY DUAL SLAVE IS UNBOUNDED-----
					...
					status = AuxLess_Dual_Slave_Unbounded(&AuxLessDualSlaveObjFunction, &AuxLessPrimalSlaveObjFunction);
					if (status != 0) {
						Found_Error("AuxLessDual_Slave_Unbounded");
						return -1;
					}

					status = GetExtremeRayOfDSP(cplexDualSlaveAuxLess_ptr);
					if (status != 0) {
						Found_Error("GetExtremeRayOfDSP");
						return -1;
					}
					...
					status = AuxLess_Dual_Slave_Bounded(cplexDualSlaveAuxLess_ptr, &DTransposeY, &AuxLessDualSlaveObjFunction, &AuxLessPrimalSlaveObjFunction, &FirstTime, loop, &NoOfTimes, modelDualSlaveAuxLess_ptr, NoofTimesWorseSolutionFound);
					if (status != 0) {
						Found_Error("Dual_Slave_Bounded");
						return -1;
					}
					...
					/*status = GetExtremePointOfDSP(cplexDualSlaveAuxLess_ptr);
					if (status != 0) {
						Found_Error("GetExtremePointOfDSP");
						return -1;
					}*/
			...
			status = Dual_Slave_Bounded(cplexDualSlave_ptr, &DTransposeY, &DualSlaveObjFunction, &PrimalSlaveObjFunction, &FirstTime, loop, &NoOfTimes, modelDualSlaveAux_ptr, modelDualSlaveAuxLess_ptr, NoofTimesWorseSolutionFound);
			if (status != 0) {
				Found_Error("Dual_Slave_Bounded");
				return -1;
			}
			...
			if (SameMasterSolution == false && NeedClassicalDSP == false) {//if we haven't found a feasible solution yet
				...
				//SOLVE THE AUXLESS (THE 2ND AUXILIARY DUAL SUBPROBLEM)
		...
		cTxAuxLess.push_back(AuxLessPrimalSlaveObjFunction);
		bTuAuxLess.push_back(AuxLessDualSlaveObjFunction);
		...
		UpperBoundAuxLessArray.push_back(UpperBoundAuxLess);
		UpperBoundGlobalAuxLessArray.push_back(UpperBoundGlobalAuxLess);
...
int PrintOptimalSolution(char* FilePath_Results_ptr, int loop_iter, int NoofTimesWorseSolutionFound) {
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_OptimalSolution.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	...
	outfile << "FeasibilityCutsFromAuxiliary2Problem(Less)= " << BDFeasCutsFromAuxLess << std::endl;
	outfile << "OptimalityCutsFromAuxiliary2Problem(Less)= " << BDOptCutsFromAuxLess << std::endl;
	outfile << "----------------------------------" << std::endl;
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_LowerBound.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_UpperBound.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_UpperBoundGlobal.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_UpperBoundAux.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_UpperBoundAuxLess.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	for (i = 0; i < UpperBoundAuxLessArray.size(); i++) {
		outfile << UpperBoundAuxLessArray.at(i) << std::endl;
	}
	outfile.close();


	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_UpperBoundGlobalAux.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_UpperBoundGlobalAuxLess.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	for (i = 0; i < UpperBoundGlobalAuxLessArray.size(); i++) {
		outfile << UpperBoundGlobalAuxLessArray.at(i) << std::endl;
	}
	outfile.close();

	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_dTransY.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_cTransX.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_bTransU.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_cTransXAux.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_cTransXAuxLess.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	for (i = 0; i < cTxAuxLess.size(); i++) {
		outfile << cTxAuxLess.at(i) << std::endl;
	}
	outfile.close();

	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_bTransUAux.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_bTransUAuxLess.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	for (i = 0; i < bTuAuxLess.size(); i++) {
		outfile << bTuAuxLess.at(i) << std::endl;
	}
	outfile.close();

	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_CurrentHeta.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_BestPrimalSlaveObjSoFar.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_BestDualSlaveObjSoFar.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_TimePath.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_SlackValuesBendersCuts.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_SlackValuesMainMasterCons.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_SameCut.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
	...
	sprintf(filepath, "%s/%s/%s_BDAuxDouble(DUAL)_SameMasterSolution.txt", FilePath_Results_ptr, FileName_Problem_ptr, FileName_Problem_ptr);
	outfile.open(filepath);
...
int main(int argc, char **argv)
{
	...
	status = BendersIteration(modelMaster, modelDualSlave, modelDualSlaveAux, modelDualSlaveAuxLess, modelDualSlaveFeasAux, &loop_iter, &NoofTimesWorseSolutionFound);
	if (status != 0) {
